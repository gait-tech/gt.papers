function results = runNeuRASparse01Experiment(dataS, dataV, ...
                        ...
                        dataX, revStepDetect, ...
                        name, setups, savedir, stepParamDir, ...
                        startFrame, endFrame, bias)
...
%     uwbMeasSigma = 0.1; %m standard deviation
    ...
    for sI=1:setupN
        ...
        if strcmp(cs.stepDetection, 'av03')
            ...
            csNSamples = size(csGfrAcc.MP, 1);
            bIsStat.MP = false(csNSamples, 1);
            bIsStat.LA = false(csNSamples, 1);
            bIsStat.RA = false(csNSamples, 1);
        end
...
%                 disp(sprintf('%.2f %.2f', rad2deg(alphaLKmin), rad2deg(alphaRKmin)));
            ...
            if ~strcmp(savedir, '')
                save(sprintf("%s/%s-%s.mat", savedir, name, cs.label), ...
                     'estBody', 'estState', 'estState2', 'bIsStat', ...
                     'runtime', 'cs')
            end
    ...
    %         results(resultsIdx) = estBody.diffRMSE(csActBody);
            ...
            % papers.explore.e22generateTech1bPlots(estBody, csActBody, intervals);
            
            for i=["MIDPEL", "LTIO", "RTIO"]
                for j=["RMSE", "Std", "Mean"]
                    results0.(sprintf("%sW%s",i,j)) = results0a.(sprintf("%s%s",i,j));
                end
            end
%         catch
%             runtime = cputime-t0;
